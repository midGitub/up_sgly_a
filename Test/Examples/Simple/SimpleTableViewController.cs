...
namespace Tacticsoft.Examples
{
    ...
    public class SimpleTableViewController : MonoBehaviour, ITableViewDataSource
    {
        ...
        void Start() {
            ...
            m_tableView.SetCellPrefab("VisibleCounterCellInstance", m_cellPrefab);
        }
        ...
        public float GetHeightForRowInTableView(TableView tableView, int row) {
            return 100;// (m_cellPrefab.transform as RectTransform).rect.height;
        }

        //Will be called by the TableView when a cell needs to be created for display
        public TableViewCell GetCellForRowInTableView(TableView tableView, int row) {
            VisibleCounterCell cell = tableView.GetReusableCell("VisibleCounterCellInstance") as VisibleCounterCell;
            if (cell == null) {
                if (cell == null)
                {
                    cell = (VisibleCounterCell)tableView.GetNewCell("VisibleCounterCellInstance");// GameObject.Instantiate(m_cellPrefab);
