...
public class AGridManager : MonoBehaviour {
    private static AGridManager s_Instance = null;
    public static AGridManager instance
    {  
        get {  
            if (s_Instance == null) {
                s_Instance = FindObjectOfType(typeof(AGridManager))
                        as AGridManager;  
                if (s_Instance == null)  
    ...
    public void GetNeighbours(ANode node, ArrayList neighbors)
    {  
    ...
    void AssignNeighbour(int row, int column, ArrayList neighbors) {  
        if (row != -1 && column != -1 &&   
            row < numOfRows && column < numOfColumns) {
                ANode nodeToAdd = nodes[row, column];  
