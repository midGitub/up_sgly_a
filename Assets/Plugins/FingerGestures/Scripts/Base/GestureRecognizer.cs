...
public abstract class GestureRecognizerTS<T> : GestureRecognizer where T : Gesture, new()
{
    ...
    public delegate void NewGestureEvent(T gesture);
    public NewGestureEvent m_gestureEvent;
    ...
    protected void RaiseEvent( T gesture )
    {
        ...
        if( UseSendMessage && !string.IsNullOrEmpty( EventMessageName ) )
        {
            ...
            {
                if (m_gestureEvent != null) m_gestureEvent.Invoke(gesture);
                else Debug.Log("please rigster stm into 'gestrue', sendmessage  has been abandoned");
            ...
            if( SendMessageToSelection != SelectionType.None )
            {
                ...
                {
                    Debug.LogError("Try to use selection, select obj :  " + sel.name);
