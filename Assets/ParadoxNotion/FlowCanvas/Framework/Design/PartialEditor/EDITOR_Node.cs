...
namespace NodeCanvas.Framework{
	...
	partial class Node{
...
#if UNITY_EDITOR
        protected virtual GUIStyle nodeGUIType()
        {
            return CanvasStyles.window;
        }
#endif
        [SerializeField]
		...
		readonly public static Vector2 minSize = new Vector2(70, 20);
		readonly public static Dictionary<Status, Color> statusColors = new Dictionary<Status, Color>()
		{
			...
			{Status.Resting, new Color(1f, 1f, 1f, 0.8f)},
		...
		public void ShowNodeGUI(Node node, Rect drawCanvas, bool fullDrawPass, Vector2 canvasMousePos, float zoomFactor){
		...
		//Draw the window
		 void DrawNodeWindow(Node node, Vector2 canvasMousePos, float zoomFactor){
			...
			node.rect = GUILayout.Window(node.ID, node.rect, (ID)=>{ NodeWindowGUI(node, ID); }, string.Empty, nodeGUIType());
		...
		static void ShowHeader(Node node){
			...
			if (!node.showIcon || node.iconAlignment != Alignment2x2.Default){
				if (node.name != null){
					...
					//}
            //sl
					var text = string.Format("<b><size=12><color=#{0}>{1}{2}</color></size></b>", "FFFFE0", (image != null? " " : ""), finalTitle);
