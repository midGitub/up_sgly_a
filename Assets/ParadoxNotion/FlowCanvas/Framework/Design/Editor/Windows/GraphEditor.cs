...
namespace NodeCanvas.Editor{
	...
	public partial class GraphEditor : EditorWindow{
		...
		//This is called while within Begin/End windows
		 void ShowNodesGUI(Graph graph, Rect drawCanvas, bool fullDrawPass, Vector2 canvasMousePos, float zoomFactor){
			for (var i = 0; i < graph.allNodes.Count; i++){
                ...
                graph.allNodes[i].ShowNodeGUI(graph.allNodes[i], drawCanvas, fullDrawPass, canvasMousePos, zoomFactor);
			}
        ...
        static List<object> currentMultiSelection = new List<object>();
			...
			if (isMultiSelecting && e.rawType == EventType.MouseUp){
                ...
                else if (e.control && !e.shift)
                {
					if (overlapedNodes.Count > 0){
						GraphEditorUtility.activeElements = currentMultiSelection.Union(overlapedNodes.Cast<object>().ToList()).ToList();
						e.Use();
					}
                ...
                else if (!e.control && e.shift)
				{
                    if (overlapedNodes.Count > 0)
                    {
                        ...
                        GraphEditorUtility.activeElements = currentMultiSelection.Except(overlapedNodes.Cast<object>().ToList()).ToList();
                        e.Use();
                    }
			...
			if (isMultiSelecting){
				...
				if (rect.width > 5 && rect.height > 5){
					...
					if (rect.width > 50 && rect.height > 50){
						...
						GUI.Label(new Rect( e.mousePosition.x + 16, e.mousePosition.y, 120, 22 ), "<i>+ control and shift for group</i>", CanvasStyles.label);
					}
				}
			}
					...
					if (handleRect.Contains(e.mousePosition)){
						...
						if (e.button == 1){
							...
							menu.AddItem(new GUIContent("Delete Group"), false, ()=>
							{
								...
								if (tempCanvasGroupNodes.Length>0 && EditorUtility.DisplayDialog("Delete Group", "Delete contained nodes as well?", "Yes", "No")){
